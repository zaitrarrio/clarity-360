...
export function Markdown({ children, className = "" }: { children: string; className?: string }) {
  return (
    <div className={`space-y-3 text-[14px] leading-[1.75] font-light text-foreground/85 ${className}`}>
      <ReactMarkdown
        components={{
          h1: (p) => <h3 className="font-display text-xl font-normal text-foreground" {...p} />,
          h2: (p) => <h3 className="font-display text-lg font-normal text-foreground" {...p} />,
          h3: (p) => <h4 className="font-display text-base font-medium text-foreground" {...p} />,
          p: (p) => <p className="text-foreground/85" {...p} />,
          ul: (p) => <ul className="list-disc space-y-1.5 pl-5 marker:text-ember" {...p} />,
          ol: (p) => <ol className="list-decimal space-y-1.5 pl-5 marker:text-ember" {...p} />,
