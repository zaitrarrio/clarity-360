  ...
  return (
    <div className={`flex flex-col overflow-hidden rounded-xl border border-border bg-card ${compact ? "h-[calc(100vh-100px)]" : "h-full"}`}>
      ...
      <form
        ...
        className="flex shrink-0 items-end gap-2 border-t border-border bg-card p-1.5"
      >
        <textarea
          value={input}
          ...
          className="max-h-24 flex-1 resize-none bg-transparent px-1 py-0.5 text-[12.5px] font-light text-foreground outline-none placeholder:text-muted-foreground/70"
        />
        <button
          type="submit"
          disabled={busy || !input.trim()}
          className="rounded-full bg-ember px-2.5 py-0.5 text-[11px] font-medium text-on-ember disabled:opacity-55"
        >
          Send
        </button>
      </form>
